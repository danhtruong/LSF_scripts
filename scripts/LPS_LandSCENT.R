...
#input <- subset(input, subset = Annotations == 'Fibroblasts' | Annotations == 'Adipocytes' | Annotations == 'Myocytes' | Annotations == 'Endothelial cells' )

input.sce <- as.SingleCellExperiment(input)
pheno.v = input.sce$Annotations
...
#compute SR
SR.o <- CompSRana(Integration.l, local = TRUE, mc.cores = 28)
saveRDS(SR.o, 'outputs/2022-02-01 LPS signaling_entropy.rds')
...
#InferLandmark.o <- InferLandmark(InferPotency.o, pheno.v = pheno.v,
                                 ...
                                 k_pam = 15)
#saveRDS(InferLandmark.o, 'outputs/2022-01-26 LPS InferLandmark.o.RDS')		
